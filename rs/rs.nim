...
proc correctMsgNoFsynd(
  ...
  erasePos: seq[int] = @[],
  erasures = false):
  (seq[GFUint], seq[GFUint]) {.raises: [ValueError, CatchableError, Defect].} =
  ...
  var
    synd = syndromes(msgOut, nsym, alpha)
  ...
  if erasePos.len > 0:
    ...
    eraseLoc = errataLocator(
      erasePos.mapIt((msgOut.len - 1 - it)),
      alpha = alpha)
    #erase_eval = errorEvaluator(synd[::-1], eraseLoc, len(eraseLoc)-1)
  ...
  if erasures:
    ...
    errLoc = errorLocator(synd, nsym, eraseLoc=eraseLoc, eraseCount=eraseCount)
    errLoc = errLoc.reversed()
    #errEval = errorEvaluator(synd[::-1], errLoc[::-1], len(errLoc)-1)[::-1] # find error/errata evaluator polynomial (not really necessary since we already compute it at the same time as the error locator poly in BM)

  # locate the message errors
  var errPos = findErrors(errLoc, len(msgOut), alpha) # find the roots of the errata locator polynomial (ie: the positions of the errors/errata)
  if errPos.len <= 0:
    raise newException(CatchableError, "Could not locate error")

  # compute errata evaluator and errata magnitude polynomials, then correct errors and erasures
  msgOut = correctErrata(msgOut, synd, errPos, alpha = alpha)
