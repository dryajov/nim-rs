...
template encodedBytes*(): untyped =
  when Exp == 8:
    ...
    @[72, 101, 108, 108, 111, 32, 82, 101,
      ...
      110, 33, 36016, 51702, 9536, 44839, 35358,
      8384, 60905, 26494, 59225, 2106]
      .mapIt( it.GFSymbol )
  elif Exp == 18:
    @[72, 101, 108, 108, 111, 32, 82, 101,
      101, 100, 45, 83, 111, 108, 111, 109, 111,
      110, 33, 168077, 13380, 24621, 141427, 54004,
      88735, 191583, 165529, 230647, 166598]
      .mapIt( it.GFSymbol )
  elif Exp == 20:
    @[72, 101, 108, 108, 111, 32, 82, 101,
      101, 100, 45, 83, 111, 108, 111, 109, 111,
      110, 33, 608501, 613582, 894931, 464752, 699334,
...
suite "Test Reed-Solomon Coding in GF " & $Exp:
  ...
  test "Decode with erasures":
    ...
    let
      (corrected, _) = decode(msg, 10, erasePos = toSeq(erased), erasures = true)
  ...
  test "Decode with errors":
    ...
    while count < 5:
      let pos = rand(0..<msg.len)
      msg[pos] = rand(0..Order.int).GFSymbol
      count += 1

    let
      (corrected, _) = decode(msg, 10)
  ...
  test "Decode with erasures and errors":
    ...
    let
      (corrected, _) = decode(msg, 10, erasePos = toSeq(erased))
    check string.fromBytes(corrected.mapIt( it.byte )) == "Hello Reed-Solomon!"
